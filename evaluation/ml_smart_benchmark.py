...
USAGE:
    ...
    python3 ml_smart_benchmark.py -p /input/ -o /output/
    ...
    #for c in contours:
        if index < 1:
            ...
            if w>img_width*0.01 and h>img_height*0.01:
                ...
                # draw contour
                trait_img = cv2.drawContours(orig, contours, -1, (0, 255, 255), line_width)
                ...
                trait_img = cv2.rectangle(orig, (x, y), (x+w, y+h), (255, 255, 0), line_width)
                ...
                trait_img = cv2.drawContours(orig, [box], 0, (255, 255, 0), line_width)
                '''
                ...
                trait_img = cv2.drawContours(orig, [hull], -1, (0, 255, 0), line_width)
                ...
                trait_img = cv2.drawContours(orig, [approx], -1, (0, 255, 0), line_width)
                ...
                trait_img = cv2.circle(orig, center, radius, (255, 0, 0), line_width)
                '''
                ...
                trait_img = cv2.circle(orig, extLeft, line_width, (255, 0, 0), -1)
                trait_img = cv2.circle(orig, extRight, line_width, (255, 0, 0), -1)
                trait_img = cv2.circle(orig, extTop, line_width, (255, 0, 0), -1)
                trait_img = cv2.circle(orig, extBot, line_width, (255, 0, 0), -1)
                
                max_width = dist.euclidean(extLeft, extRight)
                max_height = dist.euclidean(extTop, extBot)
                
                if max_width > max_height:
                    trait_img = cv2.line(orig, extLeft, extRight, (0,0,255), line_width)
                else:
                    trait_img = cv2.line(orig, extTop, extBot, (0,0,255), line_width)
...
def leaf_traits_computation(orig, labels, result_path, base_name, file_extension):
    ...
    for i in range(len(contours_rec_sorted)):
        ...
        #compute compactness
        compactness = float(cv2.contourArea(c))/cv2.contourArea(cv2.convexHull(c))
    ...
    return sum(curv_rec)/n_contours, label_trait, track_trait, leaf_index_rec, contours_rec, area_rec, curv_rec, compactness_rec, major_axis_rec, minor_axis_rec, leaf_color_ratio_rec, leaf_color_value_rec, box_coord_rec
...
def extract_traits(image_file, result_path):
    ...
    # load mask file provided by benchmark dataset
    ...
    ####################################################################################
    mask_path = os.path.dirname(abs_path) + '/' + base_name + '/' + base_name + '_mask.png'
    
    # load image data
    image_mask_loaded = cv2.imread(mask_path)
    
    print("Loading mask image : {0}\n".format(mask_path))
    
    roi_mask = cv2.threshold(cv2.cvtColor(image_mask_loaded, cv2.COLOR_BGR2GRAY), 0, 255, cv2.THRESH_BINARY)[1]
    ...
    #define result path for labeled images
    result_img_path = result_path + '/' + base_name + '/' + base_name + '_masked.png'
    cv2.imwrite(result_img_path, masked_orig)
    #####################################################################################
    ...
    if cv2.countNonZero(thresh) == 0:
        ...
        (rgb_colors, counts, hex_colors, color_ratio, masked_image_ori, segmented_image_BRG) = color_region(ROI_region, thresh, result_path, args_num_clusters)
        ...
        if args["debug"] == 1:
            ...
            # save segmentation result
            result_file = (result_path + base_name + '_seg' + file_extension)
            #print(filename)
            cv2.imwrite(result_file, thresh)
            
            # save result
            result_file = (result_path + base_name + '_plant_region' + file_extension)
            ...
            #plt.pie(counts.values(), labels = hex_colors, colors = hex_colors)
            fig_pie = plt.pie(counts.values(), labels = ratio_color, colors = hex_colors)
            ...
            labeled_img[label_hue==0] = 0
            #result_file = (result_path + base_name + '_label' + file_extension)
        ...
        #(avg_curv, label_trait, track_trait, leaf_index_rec, contours_rec, area_rec, curv_rec, compactness_rec, major_axis_rec, minor_axis_rec, leaf_color_ratio_rec, leaf_color_value_rec, box_coord_rec) = leaf_traits_computation(roi_image.copy(), labels, result_path, base_name, file_extension)
        '''
...
if __name__ == '__main__':
    ...
    for image_id, image in enumerate(imgList):
        ...
        if args["debug"] == 1:
            ...
            write_image_output(segmented_image_BRG, image_save_path, basename, '_clustered', file_extension)

            write_image_output(labeled_img, image_save_path, basename, '_label', file_extension)
            
            
            fig_pie = plt.figure(figsize = (8, 6))
            fig_pie = plt.pie(counts_values, labels = color_ratio, colors = hex_colors)
